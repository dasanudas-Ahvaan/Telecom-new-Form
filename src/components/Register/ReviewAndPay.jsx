  ...
  return (
    <section className="bg-gray-800/50 border border-gray-700 rounded-2xl p-6 space-y-6">
      <div className="flex items-center gap-3 pb-4 border-b border-gray-700">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Final step before registration
          </p>
        </div>
      </div>
      ...
      <div className="bg-gray-900/50 rounded-xl p-4 space-y-3 text-gray-300 text-sm">
        ...
        <div className="flex justify-between">
          <span>Programs</span>
          <span className="text-white">{programNames}</span>
        </div>
        <div className="flex justify-between font-semibold border-t border-gray-700 pt-2 mt-2">
          <span className="text-start">Donation Amount</span>
          <input
            type="text"
            value={contributionAmount}
            onChange={(e) =>
              setContributionAmount(e.target.value.replace(/[^0-9]/g, ""))
            }
            placeholder="Please enter an amount of your choice"
      ...
      >
        {loading ? (
          ...
          <>Pay ₹{contributionAmount} & Complete Registration</>
        )}
      </button>
